...
def get_turkish_number(num):
    ...
    if num in num_dict:
        return num_dict[num]
    tens_place_string = str(num)[0]
    tens_place_int = int(tens_place_string)
    
    ones_place_string = str(num)[-1]
    ones_place_int = int(ones_place_string)
    
    return num_dict[tens_place_int * 10] + ' ' + num_dict[ones_place_int] 